    ...
    <div className="mt-3 space-y-1.5">
      {rows.slice(0, maxItems).map((item, i) => (
        <div key={item.label} className="flex items-center gap-2">
          ...
          />
          <span className="text-[12px] text-on-surface-variant truncate flex-1">{item.label}</span>
          <span
...
export default function DonutChart({ data, unit = '', legend = true, maxLegendItems = 6, ariaLabel = 'Graphique en anneau' }) {
