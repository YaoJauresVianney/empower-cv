...
export default function HorizontalBarChart({ data, labelWidth = 130, ariaLabel = 'Graphique en barres horizontales' }) {
  const [containerRef, containerWidth] = useContainerWidth()
  const effectiveLabelWidth = Math.min(labelWidth, Math.max(80, Math.floor(containerWidth * 0.35)))

  const rows = normalizeChartData(data)
  if (!rows.length) return <ChartEmpty />

  return (
    <div role="img" aria-label={ariaLabel} ref={containerRef}>
      <ResponsiveContainer width="100%" height={Math.max(rows.length * 30, 60)}>
        <BarChart data={rows} layout="vertical" margin={{ top: 0, right: 12, left: 0, bottom: 0 }}>
          ...
          <YAxis
            type="category" dataKey="label" width={effectiveLabelWidth}
