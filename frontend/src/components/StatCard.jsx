...
export default function StatCard({ label, value, bar, trend, trendIcon, positive, animationDelay }) {
  return (
    <div
      className="animate-in rounded-2xl p-5 bg-white shadow-purple-sm"
      style={{ animationDelay }}
    >
      <p className="text-[11px] font-semibold uppercase tracking-[0.06em] mb-3 text-text-muted">
        {label}
      </p>
      <p className="text-[38px] font-bold leading-none tracking-[-0.02em] tabular-nums mb-3 text-primary">
        {value}
      </p>
      {bar !== undefined ? (
        <div>
        ...
        <div
          className={`flex items-center gap-1.5 text-[12px] font-medium ${positive ? 'text-status-success-text' : 'text-outline'}`}
        >
