export default function FilterSelect({ options, value, onChange, label }) {
  ...
  const selectLabel = label ?? normalized[0]?.label
  return (
    <div className="relative">
      <select
        aria-label={selectLabel}
        className="appearance-none rounded-lg px-3 py-1.5 pr-8 text-[13px] font-semibold outline-none cursor-pointer transition-all duration-150 max-w-[180px] truncate"
        style={{
          background: '#f4f2f7',
          color: '#4f4351',
